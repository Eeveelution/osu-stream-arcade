...
namespace osum.Input.Sources
{
    ...
    internal unsafe class InputSourceRawBase : InputSource
    {
        private class NativeWindowHook : NativeWindow
        {
            ...
            public NativeWindowHook(IntPtr handle)
            {
                AssignHandle(handle);
            }
        ...
        private void WindowsForm_OnWndProc(ref Message m)
        {
            switch (m.Msg)
            {
                case WM_TOUCH:
                {
                    int amountInputs = m.WParam.ToInt32() & 0x0000FFFF;
                    RawTouchInput[] pTouchInputs = new RawTouchInput[amountInputs];

                    if (GetTouchInputInfo(m.LParam, amountInputs, pTouchInputs, sizeof(RawTouchInput)))
                    {
                        if (OnTouch != null)
                        {
                            foreach (RawTouchInput input in pTouchInputs)
                            {
                                OnTouch(input);
                            }
                        }
                ...
                case WM_INPUT:
                {
                    ...
                    GetRawInputData(m.LParam, RawInputCommand.Input, out data, ref dwSize, sizeof(RawInputHeader));

                    switch (data.Header.Type)
                    {
                        case RawInputType.HID:
                            if (OnHID != null) OnHID(data);
                            break;
                        case RawInputType.Keyboard:
                            if (OnKeyboard != null) OnKeyboard(data);
                            break;
                        case RawInputType.Mouse:
                            if (OnMouse != null) OnMouse(data);
                            break;
                    }
        ...
        protected void bind(RawInputType rawInputType, RawInputDelegate del)
        {
            switch (rawInputType)
            {
                case RawInputType.Keyboard:
        ...
        [DllImport("user32.dll")]
        public static extern bool GetTouchInputInfo(IntPtr hTouchInput, int uiNumDevices, [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 1), Out]
            RawTouchInput[] pRawTouchInputs, int cbSize);

        [DllImport("user32.dll")]
        public static extern int CloseTouchInputHandle(IntPtr hTouchInput);
    ...
    public struct RawInput
    {
        ...
        [FieldOffset(16)] public RawInputHid Hid;
    }
